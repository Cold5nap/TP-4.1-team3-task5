      ...
      <div v-show="isStoreForm">
        ...
        <form @submit.prevent="" v-show="!loadingForm">
          <div class="row">
            ...
            <div class="col form-group rounded border m-2">
              ...
              <input
                ...
                min="0"
                v-model="markup"
                placeholder="Наценка в процентах"
                id="markup"
                class="form-control"
              />
            </div>
            ...
            <div class="col-3 form-group rounded border m-2">
              ...
              <input
                type="file"
                class="form-control form-control-file text-black"
                ref="mainImage"
                id="main_image"
              ...
              <div class="row">
                <div class="col">
                  <select class="form-control col" @change="getMaterials($event.target.value)">
                    <option v-for="page in materialPages" :key="page" value="page">
                      {{ page }}
                    </option>
                  </select>
                </div>
                <select class="form-control col" v-model="selectMaterial">
                  <option v-for="material in materials" :key="material.name" :value="material">
                    {{ material.name }}
                  </option>
                </select>
                <div class="col">
                ...
                <button class="btn-sm btn-success" @click="addMaterial">
                  <i class="bi bi-plus"></i>
                </button>
              </div>
              <div v-if="selectMaterials.length > 0" class=""><b>Использованные материалы</b></div>
              <div v-for="(row, index) in selectMaterials" :key="'selmat' + index" class="row">
                <div class="col">
                  <p>{{ row[0].name }}</p>
                </div>
                <div class="col">
          ...
          <button
            type="submit"
            @click="storeProduct"
            class="container-fluid mt-2 btn btn-lg btn-success"
          >
            Добавить композицию
          </button>
        </form>
      ...
      <table class="table table-sm table-bordered">
        ...
        <tbody>
          <tr v-for="product in products" :key="product.name">
            ...
            <td>
              <span v-for="material in product.composition" :key="material.name"
                >{{ material.name }}<br
              /></span>
            </td>
            <td>
            ...
            <td>
              <img
                height="70"
                v-bind:src="product.mainImage.path"
                v-bind:alt="product.mainImage.name"
              />
            </td>
            <td>
...
export default {
  ...
  methods: {
    ...
    getMaterials(pageNumber = 1) {
      ...
      axios
        .get("/api/adm/materials?page=" + pageNumber)
        .then((response) => {
    ...
    storeProduct() {
      ...
      axios
        .post("/api/adm/products", formData, {
          headers: {
    ...
    getProducts(pageNumber = 1) {
      ...
      this.loading = true;
      axios
        .get("/api/adm/products?page=" + pageNumber)
        .then((response) => {
